...
macro_rules! wrap_new {
    ($props:ident, $comp:ident, $widget:ident) => { {
        //already wrapped tagged Portal
...
fn build_properties<'a>(c:&'a Component) -> Properties {
    let mut props = Properties::new();
...
fn build_widget<'a>(comp:&'a Component, root_comps:&'a [Component],caller_params:Option<&'a Parameters>) -> Result<NewWidget<impl Widget + ?Sized + use<>>> {
    ...
    let v = match comp.name {
        ...
        "CheckBox" => {
            ...
            wrap_new!(props, comp, check_box )
        }
        "Flex" => {
            let flex_args = FlexArgs::from_params(&params_stack)?;

            //flex 를 만들어두고 wrap_widget 하게되면 type이 달라짐
            //즉 NewWidget 의 inner 가 달라지게 되서 오류가 나오는것으로 보이지만
            //NewWidget 에 들어가는것이 동적이 아니라 컴파일타임에 결정되어야 함
            if let Some("scroll") = comp.properties.get("overflow").and_then(|v|v.as_str()) {
                ...
                let wid = comp.id.map( |id| { unsafe { get_widget_tag(id) } } );
                let wopts = WidgetOptions::default();
                let flex = Portal::new( NewWidget::new_with(flex , wid, wopts, props)  );
        ...
        "Grid" => {
            ...
            if let Some("scroll") = comp.properties.get("overflow").and_then(|v|v.as_str()) {
                ...
                let mut grid = Portal::new( NewWidget::new(grid) );
        ...
        "ProgressBar" => {
            ...
            wrap_new!(props, comp, progress_bar )
        }
        "Prose" => {
        ...
        "Slider" => {
            ...
            wrap_new!(props, comp, slider )
        }
        "Spinner" => {
            let spinner = Spinner::new();
            wrap_new!(props, comp, spinner )
        }
        "Split" => {
        ...
        "TextInput" => {
            ...
            wrap_new!(props, comp, text_input )
        }
        "VariableLabel" => {
            let text = "";
            let var_label = VariableLabel::new(text);
            wrap_new!(props, comp, var_label )
        }
        name @ _ => {
